 ...
 * inside async tool handlers (e.g. after awaiting a Datto BCDR API call,
 * before sending an elicitation prompt back through "the" server).
 *
 ...
 * server — so A's elicitation prompt is sent down B's connection instead of
 * A's (or vice versa, depending on timing).
 *
