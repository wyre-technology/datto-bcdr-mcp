 ...
 * `await` gaps inside async tool handlers (e.g. after awaiting a Datto
 * BCDR API call, before sending an elicitation prompt back through "the"
 * server).
 *
 ...
 * server — so A's elicitation prompt is sent down B's connection instead
...
describe("server-ref cross-tenant isolation", () => {
  ...
  it(
    ...
    async () => {
      ...
      // (simulating an in-flight Datto BCDR API call inside a tool
      // handler) BEFORE sending its elicitation prompt.
      const tenantA = runWithServerRef(serverA, async () => {
        ...
        return elicitText("Confirm tenant A's sensitive action?", "value");
      });
      ...
      const tenantB = runWithServerRef(serverB, async () => {
        return elicitText("Confirm tenant B's sensitive action?", "value");
      });
      await tenantB;
